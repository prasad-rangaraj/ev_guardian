import pandas as pd
import numpy as np
from config import RAW_SENSORS, ENGINEERED_FEATURES
...
def compute_features(df_input):
    ...
    on the input DataFrame for the Sensor Trust Engine.
    """
...
def compute_physical_features(df_input):
    ...
    and status one-hot encodings for the LSTM Fault Classifier.
    """
    df = df_input.copy().reset_index(drop=True)
